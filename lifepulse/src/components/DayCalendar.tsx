...
type DayEventWithPosition = (Task | CalendarEvent) & {
  top: number;
  height: number;
  left: number;
  width: number;
...
export default function DayCalendar({
  ...
  // Day-specific event positioning logic
  const calculateDayEventPositions = useCallback((events: (Task | CalendarEvent)[], date: Date): DayEventWithPosition[] => {
    ...
    const positionedEvents: DayEventWithPosition[] = [];
    const columns: { start: number; end: number; events: DayEventWithPosition[] }[] = [];
    ...
    sortedEvents.forEach(event => {
      ...
      if (columnIndex === -1) {
        ...
        columns.push({ start: startMinutes, end: endMinutes, events: [] });
      } else {
      ...
      const positionedEvent: DayEventWithPosition = {
        ...event,
        top: topPosition,
        height: finalHeight,
        left: columnIndex, // Initial column position
    ...
    const overlaps = (a: DayEventWithPosition, b: DayEventWithPosition) => {
      const aStart = a.top;
    ...
    const visited = new Set<DayEventWithPosition>();
    positionedEvents.forEach(evt => {
      if (visited.has(evt)) return;
      
      // BFS to collect overlap cluster
      const group: DayEventWithPosition[] = [];
      const stack: DayEventWithPosition[] = [evt];
      ...
      while (stack.length) {
        ...
        positionedEvents.forEach(other => {
  ...
  // Get events for the day with positioning
  const dayEvents = useMemo(() => {
    const allEvents = [...tasks, ...calendarEvents];
    return calculateDayEventPositions(allEvents, currentDate);
  }, [tasks, calendarEvents, currentDate, calculateDayEventPositions]);
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-900">
      {/* Day Header */}
      <div className="p-6 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 sticky top-0 z-20">
        <div className={`text-2xl font-bold text-center ${isToday(currentDate) ? 'text-blue-700 dark:text-blue-400' : 'text-gray-900 dark:text-white'}`}>
          {format(currentDate, 'EEEE')}
        </div>
        <div className={`text-lg text-center mt-1 ${isToday(currentDate) ? 'text-blue-600 dark:text-blue-300' : 'text-gray-600 dark:text-gray-400'}`}>
        ...
        <div className="relative min-h-full">
          {timeSlots.map(hour => (
            <div key={hour} className="flex relative" style={{ height: HOUR_HEIGHT }}>
              {/* Time Column */}
              <div className="w-20 border-r border-gray-200 dark:border-gray-700 relative">
                {hour > 0 && hour <= 23 && (
              ...
              {/* Events Column Container with Grid Line */}
              <div className={`flex-1 relative ${hour < 23 ? 'border-b border-gray-100 dark:border-gray-700/30' : ''}`}>
                {/* Events Column */}
                <div 
                  className="flex-1 relative hover:bg-gray-50 dark:hover:bg-gray-800/20 cursor-pointer transition-colors group"
          ...
          {dayEvents.map((event, eventIndex) => {
            ...
            // Day-specific positioning: event.left and event.width directly
            const leftPercentage = event.left;
            const widthPercentage = event.width;
            ...
            return (
              ...
              >
                <div className="px-1 py-0.5 h-full flex flex-col justify-center overflow-hidden">
                  ...
                  {event.height > 45 && event.description && isCalendarEvent(event) && (
